...
class DenoiseDataSet(Dataset):
    def __init__(self, add_noise: str = None, crop_size: list = None, aug: list = None, n_repeat: int = 1,
                 n_data: int = None, ratio_data: float = None) -> None:
        '''
        ...
        # self.dataset_dir = '/oldhome/wry/AP-BSN-master/dataset'
        self.dataset_dir = '/DATA2/wry/dataset'
    ...
    def __getitem__(self, idx):
        ...
        if self.add_noise_type is not None:
            if 'clean' in data:
                syn_noisy_img, nlf = self._add_noise(data['clean'], self.add_noise_type, self.add_noise_opt,
                                                     self.add_noise_clamp)
                data['syn_noisy'] = syn_noisy_img
                data['nlf'] = nlf
            elif 'real_noisy' in data:
                syn_noisy_img, nlf = self._add_noise(data['real_noisy'], self.add_noise_type, self.add_noise_opt,
                                                     self.add_noise_clamp)
                data['syn_noisy'] = syn_noisy_img
                data['nlf'] = nlf
            else:
                raise RuntimeError(
                    'there is no clean or real image to synthesize. (synthetic noise type: %s)' % self.add_noise_type)

        # data augmentation
        if self.aug is not None:
            data = self._augmentation(data, self.aug)
            # data = self._random_crop(data, self.aug)
    ...
    def _get_patch(self, crop_size, data, rnd=True):
        ...
        if 'clean' in data and 'real_noisy' in data:
            assert data['clean'].shape[1] == data['clean'].shape[1] and data['real_noisy'].shape[2] == \
                ...
                data['clean'].shape[1], data['clean'].shape[1], data['real_noisy'].shape[2],
                data['real_noisy'].shape[2])
    ...
    # def _random_crop(self, data: dict, crop_size: tuple):
    #     """
    #     Randomly crops the images in the data dictionary to the specified crop size.
    #     """
    #     for key in data:
    #         if self._is_image_tensor(data[key]):
    #             # Get the size of the image tensor
    #             image_size = data[key].size()
    #
    #             if len(image_size) != 3:
    #                 raise ValueError("Image tensor should have 3 dimensions (C, H, W).")
    #
    #             # Check if the crop size is smaller than the image size
    #             if crop_size[0] > image_size[1] or crop_size[1] > image_size[2]:
    #                 raise ValueError("Crop size should be smaller than image size.")
    #
    #             # Calculate the maximum left and top coordinates for cropping
    #             max_left = image_size[2] - crop_size[1]
    #             max_top = image_size[1] - crop_size[0]
    #
    #             # Generate random left and top coordinates
    #             left = random.randint(0, max_left)
    #             top = random.randint(0, max_top)
    #
    #             # Crop the image tensor
    #             data[key] = data[key][:, top:top + crop_size[0], left:left + crop_size[1]]
    #
    #     return data
    ...
    def prep_save(self, img_idx: int, img_size: int, overlap: int, clean: bool = False, syn_noisy: bool = False,
                  real_noisy: bool = False):
        '''
...
class MergedDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        raise RuntimeError(
            'index of merged dataset contains some bugs, total length %d, requiring idx %d' % (self.__len__(), idx))
