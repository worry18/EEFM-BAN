...
def pixel_shuffle_down_sampling(x:torch.Tensor, f:int, pad:int=0, pad_value:float=0.):
    ...
    if len(x.shape) == 3:
        ...
        # 利用F.pad函数对unshuffled进行填充
        if pad != 0: unshuffled = F.pad(unshuffled, (pad, pad, pad, pad), value=pad_value)

        # w//f+2*pad表示计算下采样后图像的宽度：下采样可能会导致图像边缘的像素丢失或者不完整。为了保持图像边缘的完整性，需要进行填充。
        # .permute(0,1,3,2,4) 将张量的维度重新排列
        # .reshape(c, w+2*f*pad, h+2*f*pad)将张量形状再次变换，得到最终的下采样图像张量。
